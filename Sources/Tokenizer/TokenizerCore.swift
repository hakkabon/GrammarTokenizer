...
//  │       │                                                                 │
//  │  TokenizerCore (open class)   — shared scanning engine (no buffer)      │
//  │       │                                                                 │
...
//  │  TokenizerCore itself has no internal buffer; it is a pure scanner.     │
//  │  Callers that need peek / consume wrap any TokenizerCore subclass in    │
...
///
/// `TokenizerCore` owns the `UnicodeScalarView` cursor, the symbol `Trie`, and
...
open class TokenizerCore: Tokenizing {
    ...
    public init(_ source: String, symbols: Set<String> = [], keywords: Set<String> = []) {
        ...
        self.symbols    = TokenizerCore.builtInSymbols.union(symbols)
    ...
    // MARK: - tokenize() with residual-input guard
